...
public class PhisRunnableRandom implements Runnable{
    ...
    public PhisRunnableRandom(BigInteger start, BigInteger end, StandardInput input){
        this.start = start;
        this.end = end;
        this.nodes = new ArrayList<>(input.getNodes());
    ...
    public void run() {
        ...
        Network network = new Network(nodes, startNodeIndex, goalNodeIndex);

        while (index.compareTo(end) < 0) {
            String binary = String.format("%" + nodes.size() + "s", generateRandomBigInt(index, end).toString(2)).replace(' ', '0');
